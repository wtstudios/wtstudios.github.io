    ...
    animateBetweenTicks = setInterval(function() {
      if(assetsAreLoaded && state.includes("ingame")) {
        ...
        for(let i = 0; i < gameData.particles.length; i++) {
          ...
          switch(particleData.type) {
            case "cartridge":
              ...
              break;
            case "residue":
              particleData.position.x += Math.cos(particleData.angle) * (particleData.opacity / 5) * 0.275;
              particleData.position.y += Math.sin(particleData.angle) * (particleData.opacity / 5) * 0.275;
        ...
        for (let i = 0; i < gameData.users.length; i++) {
          ...
          player.state.position.x += player.state.force.x * 0.275;
          player.state.position.y += player.state.force.y * 0.275;
          if(gameData.users[i] == permanentID && gameData.players[permanentID].health > 0) {
            queuedCameraLocation.x += player.state.force.x * 0.275;
            queuedCameraLocation.y += player.state.force.y * 0.275;
            queuedCameraLocation.targetX += player.state.force.x * 0.275;
            queuedCameraLocation.targetY += player.state.force.y * 0.275;
          }
        }
      }
    }, 25)
